# Архитектура и назначение выводов

## Разделение системы

Неподвижная часть: Li-Po 3S, предохранитель, выключатель, ESC и A2212. Вращающаяся часть Rev B: MP1584, ESP32, KY-003, SN74AHCT125 и две лопасти APA102.

```mermaid
flowchart LR
  BAT[Li-Po 3S] --> F1[Главный предохранитель]
  F1 --> SW[Выключатель]
  SW --> ESC[ESC 30 A]
  ESC --> M[A2212 1000KV]
  SW --> F2[Предохранитель логики]
  F2 --> SR[Кольцо: 1+2 = VBAT; 3+4 = GND]
  SR --> BUCK[MP1584 5 V на роторе]
  BUCK --> ESP[ESP32 на роторе]
  ESP --> BUF[SN74AHCT125]
  BUF --> A[APA102 A: 20 LED default]
  BUF --> B[APA102 B: 20 LED default]
  H[KY-003] --> ESP
  ESP -->|GPIO25, канал 5| ESC
```

## Шестиканальное кольцо

| Канал | Предварительное назначение | Условие |
|---:|---|---|
| 1 | VBAT_ROTOR | Параллельно каналу 2 только после проверки каждого канала |
| 2 | VBAT_ROTOR | То же |
| 3 | GND_ROTOR | Параллельно каналу 4 после проверки |
| 4 | GND_ROTOR | То же |
| 5 | ESC_PWM | GPIO25 → 220 Ом → роторный конец → статорный ESC |
| 6 | Резерв | Оба конца изолировать |

Параллельные контакты могут делить ток неравномерно. Окончательное разрешение зависит от сопротивления каналов и нагрузочного теста.

## SN74AHCT125N

| Канал микросхемы | Вход | Выход | Назначение |
|---|---:|---:|---|
| 1 | pin 2 | pin 3 | GPIO18 → DATA A |
| 2 | pin 5 | pin 6 | GPIO19 → CLOCK A |
| 3 | pin 9 | pin 8 | GPIO23 → DATA B |
| 4 | pin 12 | pin 11 | GPIO22 → CLOCK B |

Pins 1, 4, 10 и 13 (`OE`) соединить с GND. Pin 14 → 5 В, pin 7 → GND. Конденсатор 100 нФ установить непосредственно между pins 14 и 7. На каждом из четырёх выходов нужен последовательный резистор 330–470 Ом.

## Питание лопастей

Питание обеих лопастей разводится звездой от центральной точки ротора. Нельзя пропускать питание второй лопасти через первую. У входа каждой ленты требуется локальная развязка; номиналы уточняются после измерения пульсаций и длины проводов.

## Разъёмы

Назначение контактов `JH1`, `JLA`, `JLB` и `JESC`, проектные цвета и критерии качества обжима собраны в [карте разъёмов](../hardware/connectors-and-wire-colors.md). Машиночитаемая копия распиновки находится в `hardware/connector-pinout.csv`.

XH-совместимые корпуса не применяются в силовой цепи аккумулятора и мотора. Возможность их использования для питания светодиодных лопастей требует отдельного нагрузочного теста.
